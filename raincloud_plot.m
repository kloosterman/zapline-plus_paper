...
addOptional(p, 'line_width', 1.5, validScalarPosNum)
...
for i_column = 1:size(data,2)
	...
	if box_on
		...
		% whiskers
		handles{i_column,5} = line([quantiles(2) quantiles(5)], [nanmean(box_edges) nanmean(box_edges)], 'col', bxcl, 'LineWidth', line_width);
		handles{i_column,6} = line([quantiles(1) quantiles(4)], [nanmean(box_edges) nanmean(box_edges)], 'col', bxcl, 'LineWidth', line_width);
...
if plot_means
	plot(nanmean(data),1:size(data,2),'ko','markerfacecolor','k')
	
	for data_part = 1:size(data,2)
		
		plot([nanmean(data(:,data_part))-nanstd(data(:,data_part)); nanmean(data(:,data_part))+nanstd(data(:,data_part))],[data_part data_part],'k','linewidth',line_width);
